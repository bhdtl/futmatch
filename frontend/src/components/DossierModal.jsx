...
Ihr FutMatch Executive Advisor Team`;
  ...
  return (
    <div class="fixed inset-0 bg-zinc-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div class="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 relative">
        <button onClick={onClose} class="absolute top-4 right-4 text-zinc-400 hover:text-white p-1">
          ✕
        </button>

        <div class="flex items-center gap-3 border-b border-zinc-800 pb-4">
          <div class="w-10 h-10 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-sm font-bold text-emerald-400">
            {logo}
          </div>
          <div>
            <span class="text-[10px] text-emerald-400 font-mono uppercase tracking-wider">FutMatch Executive Dossier</span>
            <h3 class="text-lg font-bold text-white">{name}</h3>
            <p class="text-xs text-zinc-400">{club.league}</p>
          </div>
        </div>
        ...
        <div class="space-y-3 text-xs">
          <div class="bg-zinc-950 p-3.5 rounded-xl border border-zinc-800 space-y-1.5">
            <div class="flex justify-between items-center text-zinc-300">
              ...
              <span class="text-emerald-400 font-bold">{score}% Match</span>
            </div>
          ...
          <div class="space-y-1">
            <label class="text-zinc-300 font-medium">Vorbereiteter Direkt-Pitch für Sportdirektor:</label>
            <textarea 
              readOnly 
              value={pitchText} 
              class="w-full h-28 bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-zinc-300 font-mono text-[11px] focus:outline-none custom-scrollbar"
            />
          </div>
        </div>

        <div class="flex items-center justify-end gap-3 pt-2">
          <button onClick={onClose} class="px-4 py-2 bg-zinc-800 text-zinc-300 text-xs font-medium rounded-xl">Schließen</button>
          <button 
            onClick={() => alert(`Dossier für ${name} erfolgreich als PDF exportiert!`)} 
            class="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 text-xs font-bold rounded-xl shadow-lg shadow-emerald-500/20"
          >
            PDF Herunterladen
          </button>
        </div>
