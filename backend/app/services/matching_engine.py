...
Supports "ALL" / "Beliebig" wildcards for every parameter.
"""
...
class MatchingEngine:
    ...
    def calculate_matches(profile: ClientProfileRequest) -> List[ClubMatchResponse]:
        ...
        pos_raw = (profile.position or "ALL").upper()
        ...
        for club in clubs_data:
            ...
            total_expiring = expiring_def + expiring_mid + expiring_att

            relevant_expiring_count = 0
            relevant_players_str = ""

            if pos_raw in ["ALL", "BELIEBIG", ""]:
                relevant_expiring_count = total_expiring
                all_exp_players = (expiring_map.get("defenders", []) + expiring_map.get("midfielders", []) + expiring_map.get("attackers", []))
                relevant_players_str = ", ".join(all_exp_players[:3]) if all_exp_players else "Kaderweiterung 2026/27"
            elif pos_raw in ["IV", "LV", "RV", "CB", "LB", "RB", "TW"]:
            ...
            elif pos_raw in ["ZM", "DM", "OM", "CM", "CAM", "CDM"]:
            ...
            if pos_raw in ["IV", "LV", "RV"] and any("3-" in t for t in primary_tactics):
                tactical_score += 20
            elif pos_raw in ["ZM", "DM"] and any("4-2-3-1" in t or "4-3-3" in t for t in primary_tactics):
                tactical_score += 15
            elif pos_raw in ["MS", "LF", "RF"] and any("3-4-2-1" in t or "4-3-3" in t for t in primary_tactics):
                tactical_score += 15
            ...
            # 2. Vacancy Score (35%)
            vacancy_score = 55
            if relevant_expiring_count >= 3:
                vacancy_score += 40
            elif relevant_expiring_count >= 1:
                vacancy_score += 25

            if not profile.contract_status or profile.contract_status in ["summer2027", "free", "ALL", "BELIEBIG"]:
                vacancy_score += 10
            ...
            if not profile.age or ideal_min <= profile.age <= ideal_max:
                attribute_score += 20
            ...
            final_score = max(72, min(final_score, 98))
            ...
            urgency = "Sehr Hoch" if relevant_expiring_count >= 2 else "Normal"
